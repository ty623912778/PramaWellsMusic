...
static NSString * const PlayerTracksKey = @"tracks";
static NSString * const PlayerPlayableKey = @"playable";
static NSString * const PlayerDurationKey = @"duration";


// KVO Player Keys
static NSString * const PlayerRateKey = @"rate";  // rate
// KVO Player Item Keys
static NSString * const PlayerStatusKey = @"status"; // 状态
static NSString * const PlayerEmptyBufferKey = @"playbackBufferEmpty"; // 缓冲操作
static NSString * const PlayerPlayerKeepUpKey = @"playbackLikelyToKeepUp";
static NSString * const PlayerPlayerBufferFullKey = @"playbackBufferFull";
// KVO Player Preload Keys
static NSString * const PlayerPlayerLoadedTimeRanges = @"loadedTimeRanges";  // 缓冲进度

// KVO Contexts
static NSString * const PlayerObserverContext = @"PlayerObserverContext";
static NSString * const PlayerItemObserverContext = @"PlayerItemObserverContext";
static NSString * const PlayerPreloadObserverContext = @"PlayerPreloadObserverContext";


@interface Player ()<PlayViewBottomViewDelegate,PlayerViewDelegate>
{
@private
...
- (void)playerViewDidClickBackButton{
    if (self.orientation == UIDeviceOrientationLandscapeLeft || self.orientation == UIDeviceOrientationLandscapeRight) {
        [self PlayViewBottomView:self.playerView.bottomView didClcikFullScreenButton:self.playerView.bottomView.fullScreenButton];
    }
}
...
- (void)addObserverWithPlayer:(AVPlayer *)player {
    ...
    [player addObserver:self forKeyPath:PlayerRateKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)PlayerObserverContext];
    // Player Observer
...
- (void)removeObserverWithPlayer:(AVPlayer *)player {
    [player removeObserver:self forKeyPath:PlayerRateKey context:(__bridge void *)PlayerObserverContext];
...
- (void)removeObserverWithPlayerItem:(AVPlayerItem *)playerItem {
    [playerItem removeObserver:self forKeyPath:PlayerEmptyBufferKey context:(__bridge void *)PlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:PlayerPlayerKeepUpKey context:(__bridge void *)PlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:PlayerPlayerBufferFullKey context:(__bridge void *)PlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:PlayerStatusKey context:(__bridge void *)PlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:PlayerPlayerLoadedTimeRanges context:(__bridge void *)PlayerPreloadObserverContext];
}
...
- (void)addObserverWithPlayerItem:(AVPlayerItem *)playerItem {
    ...
    [playerItem addObserver:self forKeyPath:PlayerStatusKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(PlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:PlayerPlayerLoadedTimeRanges options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(PlayerPreloadObserverContext)];
    [playerItem addObserver:self forKeyPath:PlayerEmptyBufferKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(PlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:PlayerPlayerKeepUpKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(PlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:PlayerPlayerBufferFullKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(PlayerItemObserverContext)];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if (context == (__bridge void *)(PlayerObserverContext)) {
        // Player KVO
        if ([keyPath isEqualToString:PlayerRateKey]) {
    ...
    } else if (context == (__bridge void *)(PlayerItemObserverContext)) {
        // PlayerItem KVO
        if ([keyPath isEqualToString:PlayerEmptyBufferKey]) {
            if (self.playerItem.playbackBufferEmpty) {
                _userPause = NO;
                self.bufferingState = PlayerBufferingStateDelayed;
            }
        } else if ([keyPath isEqualToString:PlayerPlayerKeepUpKey]) {
            if (self.playerItem.playbackLikelyToKeepUp) {
                
                self.bufferingState = PlayerBufferingStateKeepUp;
            }
        } else if ([keyPath isEqualToString:PlayerPlayerBufferFullKey]) {
            if (self.playerItem.playbackBufferFull) {
                self.bufferingState = PlayerBufferingStateFull;
            }
        }else if ([keyPath isEqualToString:PlayerStatusKey]) {
    ...
    } else if (context == (__bridge void *)PlayerPreloadObserverContext) {
        if ([keyPath isEqualToString:PlayerPlayerLoadedTimeRanges]) {
