...
@implementation PlayViewBottomView
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [controlButton setImage:[UIImage imageNamed:VideoName(@"player_bofang")] forState:UIControlStateNormal];
        controlButton.selected = YES;
        [controlButton setImage:[UIImage imageNamed:VideoName(@"player_pause")] forState:UIControlStateSelected];
        ...
        PlayerProgressView *progressView = [PlayerProgressView initilzerProgressViewWithFrame:CGRectMake(0, 10, 375, 20)];
        ...
        [fullScreenButton setImage:[UIImage imageNamed:VideoName(@"player_fullscreen")] forState:UIControlStateNormal];
        [fullScreenButton setImage:[UIImage imageNamed:VideoName(@"player_embeddedscreen")] forState:UIControlStateSelected];
...
- (void)controlButtonClick:(UIButton *)controlButton{
    if ([self.delegate respondsToSelector:@selector(PlayViewBottomView:didClcikControlButton:)]) {
        [self.delegate PlayViewBottomView:self didClcikControlButton:controlButton];
    }
}
- (void)fullScreenButtonClcik:(UIButton *)fullScreenButton{
    if ([self.delegate respondsToSelector:@selector(PlayViewBottomView:didClcikFullScreenButton:)]) {
        [self.delegate PlayViewBottomView:self didClcikFullScreenButton:fullScreenButton];
    }
}
- (void)sliderChangeValue:(PlayerProgressView *)progressView{
    if ([self.delegate respondsToSelector:@selector(PlayViewBottomView:didUpdateProgressView:)]) {
        [self.delegate PlayViewBottomView:self didUpdateProgressView:progressView];
    }
}
- (void)positionSliderUp:(PlayerProgressView *)progressView{
    if ([self.delegate respondsToSelector:@selector(PlayViewBottomView:sliderPositionSliderUp:)]) {
        [self.delegate PlayViewBottomView:self sliderPositionSliderUp:progressView];
    }
}
- (void)positionSliderDown:(PlayerProgressView *)progressView{
    if ([self.delegate respondsToSelector:@selector(PlayViewBottomView:sliderPositionSliderDown:)]) {
        [self.delegate PlayViewBottomView:self sliderPositionSliderDown:progressView];
    }
}
@end
