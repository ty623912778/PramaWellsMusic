...
@property (nonatomic, strong) ForwardView *forwardView;
...
- (void)setUpTopView{
    ...
    PlayerViewTopView *topView = [[PlayerViewTopView alloc] init];
...
- (void)setupBottomView{
    PlayViewBottomView *bottomView = [[PlayViewBottomView alloc] init];
    ...
    if (gesture.state == UIGestureRecognizerStateChanged )
    {
        ...
        switch (self.direction) {
            ...
            case PanDirectionRight: // 向右划(快进)
            {
                ...
                self.forwardView.mode = ForwardViewModeForward; // 快进模式
                ...
                if (panLength > 0) {
                    ...
                    // 否则开始向左划 快退显示
                    self.forwardView.mode = ForwardViewModeRewind;
                    CGPoint endPoint = [gesture locationInView:gesture.view];
                    int panLength = endPoint.x - self.startPoint.x;
            ...
            case PanDirectionLeft: // 向左划 快退手势
            {
                ...
                if (panLength <  0) {
                    ...
                    self.forwardView.mode = ForwardViewModeForward;
...
- (ForwardView *)forwardView{
    if (!_forwardView) {
        _forwardView = [[ForwardView alloc] init];
